classdef Controlador < handle
    ...
    methods(Static)
        ...
        function [fila,columna] = primeraJugada()
            ...
            [fila,columna] = Controlador.escogerEsquinaAleatoria();
            return
        end
        ...
        function [fila,columna] = segundaJugada(matriz)
            ...
            if(matriz == [0 0 0;0 2 0;0 0 0]) %#ok<BDSCA> es para suprimir warnings
                % centro
                [fila,columna] = Controlador.escogerEsquinaAleatoria();
                return
            elseif(matriz == [0 2 0;0 0 0;0 0 0]) %#ok<BDSCA>
                % lado 1
                fila=1;
                columna=3;
                return
            elseif(matriz == [0 0 0;0 0 2;0 0 0]) %#ok<BDSCA>
                % lado 2
                fila=3;
                columna=3;
                return
            elseif(matriz == [0 0 0;0 0 0;0 2 0]) %#ok<BDSCA>
                % lado 3
                fila=3;
                columna=3;
                return
            elseif(matriz == [0 0 0;2 0 0;0 0 0]) %#ok<BDSCA>
                % lado 4
                fila=3;
                columna=1;
                return
            elseif(matriz == [2 0 0;0 0 0;0 0 0]) %#ok<BDSCA>
                % esquina 1
                fila=2;
                columna=2;
                return
            elseif(matriz == [0 0 2;0 0 0;0 0 0]) %#ok<BDSCA>
                % esquina 2
                fila=2;
                columna=2;
                return
            elseif(matriz == [0 0 0;0 0 0;0 0 2]) %#ok<BDSCA>
                % esquina 3
                fila=2;
                columna=2;
                return
            elseif(matriz == [0 0 0;0 0 0;2 0 0]) %#ok<BDSCA>
                % esquina 4
                fila=2;
                columna=2;
                return
        ...
        function [f,c] = contraJugadas(matriz)
            ...
            for i=1:4
                if(matriz == [2 0 0;0 1 0;0 0 2]) %#ok<BDSCA>
                    fila=1;
                    columna=2;
                    break;
                end
