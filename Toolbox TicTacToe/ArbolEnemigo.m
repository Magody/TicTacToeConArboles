classdef ArbolEnemigo < handle
    ...
    methods
        ...
        function arreglo = generarNodos(obj, matriz, turno, nivel)
            ...
            if(ControladorEnemigo.noTieneCeros(matriz))
            ...
            for i=1:length(matriz)
                ...
                if(siguiente_matriz(i) == 0)
                    ...
                    puntero.agregarContenido(ControladorEnemigo.convertirVectorMatriz3x3(siguiente_matriz));
                    
                    puntero.posicion = ControladorEnemigo.posicionJugada(i);
                    puntero.nivel = nivel;
                    
                    % CONDICION DE RECORTE
                    estado = ControladorEnemigo.analizarEstado(puntero.contenido);
                    if(estado ~= 0)
                        %recorte del arbol, si ya gana alguien, no genera mas hs ni hijos
                        espacios_disponibles = ControladorEnemigo.contarCeros(ControladorEnemigo.convertirVectorMatriz3x3(siguiente_matriz));
                        puntero.peso = estado + ControladorEnemigo.calcularNodosHijos(espacios_disponibles)*estado;
                        if(nivel == 1 || nivel == 2)
                    ...
                    puntero.l = obj.generarNodos(siguiente_matriz, ControladorEnemigo.alternarTurno(turno), nivel+1);
                    ...
                    if(~ControladorEnemigo.noTieneCeros(siguiente_matriz))
                        % Este if permite no crear un arbol al terminal de todos los nodos
                        puntero.h = ArbolEnemigo();
                        puntero = puntero.h; % recorre para agregar todo a sus hs
                    elseif(ControladorEnemigo.noTieneCeros(siguiente_matriz))
                        % Aqui solo entran los nodos totalmente terminales. Y son la base para ir autosumando
                        puntero.peso = ControladorEnemigo.analizarEstado(ControladorEnemigo.convertirVectorMatriz3x3(siguiente_matriz));
                    end
                end
            end
        end
